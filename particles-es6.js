/**
 * particles-engine.js
 * A lightweight, documented particle engine with density clamping, spatial grid line-linking,
 * and interactive modes (grab, bubble, repulse, push, remove). Works on any container element.
 */
/** Typedefs omitted for brevity in this header (see full JSDoc below). */
...
export class ParticleJS {
  ...
  _spawnParticle() {
    ...
    if (p.shape.type === 'image' && p.shape.image?.src) { this._loadImage(p.shape.image.src).then(img => part.image = img); }
    return part;
  }
  _step(dt) {
    const ctx = this.ctx;
    ctx.save(); ctx.clearRect(0, 0, this.canvas.width, this.canvas.height); ctx.restore();
    for (const particle of this.particles) { this._updateParticle(particle, dt); this._drawParticle(ctx, particle); }
  ...
  _updateParticle(p, dt) {
    ...
    if (this.pointer.active && this.options.interactivity.events.onhover.enable) {
      ...
      } else { p.size += (p.baseSize - p.size) * Math.min(1, dt * 4); p.opacity += (p.baseOpacity - p.opacity) * Math.min(1, dt * 4); }
    }
    const clickSince = (performance.now() - this.pointer.lastClick) / 1000;
    const clickBubble = this.options.interactivity.events.onclick.enable && this.options.interactivity.events.onclick.mode === 'bubble' && clickSince < (this.options.interactivity.modes.bubble.duration);
    if (clickBubble) {
      const dx = p.x - this.pointer.x; const dy = p.y - this.pointer.y; const dist = Math.hypot(dx, dy); const m = this.options.interactivity.modes;
      if (dist < m.bubble.distance * this.dpr) { const k = 1 - dist / (m.bubble.distance * this.dpr); p.size = p.baseSize + (m.bubble.size - p.baseSize) * k; p.opacity = p.baseOpacity + (m.bubble.opacity - p.baseOpacity) * k; }
    }
    ...
    if (this.options.particles.move.out_mode === 'bounce') {
      ...
      p.x = Math.max(0, Math.min(this.canvas.width, p.x)); p.y = Math.max(0, Math.min(this.canvas.height, p.y));
    } else {
      if (p.x < -50) p.x = this.canvas.width + 50; if (p.x > this.canvas.width + 50) p.x = -50; if (p.y < -50) p.y = this.canvas.height + 50; if (p.y > this.canvas.height + 50) p.y = -50;
    }
  }
  _drawParticle(ctx, p) {
    ctx.save(); ctx.globalAlpha = Math.max(0, Math.min(1, p.opacity)); ctx.fillStyle = p.fillStyle;
    if (p.strokeStyle) { ctx.strokeStyle = p.strokeStyle; ctx.lineWidth = this.options.particles.shape.stroke.width * this.dpr; }
    const type = this.options.particles.shape.type;
    switch (type) {
      case 'circle': ctx.beginPath(); ctx.arc(p.x, p.y, p.size * this.dpr, 0, Math.PI * 2); ctx.closePath(); if (p.strokeStyle) ctx.stroke(); ctx.fill(); break;
      case 'edge': { const s = p.size * this.dpr; ctx.beginPath(); ctx.rect(p.x - s/2, p.y - s/2, s, s); if (p.strokeStyle) ctx.stroke(); ctx.fill(); ctx.closePath(); break; }
      case 'triangle': { const s = p.size * this.dpr; ctx.beginPath(); ctx.moveTo(p.x, p.y - s / Math.sqrt(3)); ctx.lineTo(p.x - s / 2, p.y + s / (2 * Math.sqrt(3))); ctx.lineTo(p.x + s / 2, p.y + s / (2 * Math.sqrt(3))); ctx.closePath(); if (p.strokeStyle) ctx.stroke(); ctx.fill(); break; }
      case 'polygon': { const sides = Math.max(3, this.options.particles.shape.polygon.nb_sides ?? 5); const r = p.size * this.dpr; ctx.beginPath(); for (let i = 0; i < sides; i++) { const a = (i / sides) * Math.PI * 2; const x = p.x + r * Math.cos(a); const y = p.y + r * Math.sin(a); if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.closePath(); if (p.strokeStyle) ctx.stroke(); ctx.fill(); break; }
      case 'image': if (p.image && p.image.complete) { const w = (this.options.particles.shape.image.width ?? p.size) * this.dpr; const h = (this.options.particles.shape.image.height ?? p.size) * this.dpr; ctx.drawImage(p.image, p.x - w/2, p.y - h/2, w, h); } else { ctx.beginPath(); ctx.arc(p.x, p.y, p.size * this.dpr, 0, Math.PI * 2); ctx.closePath(); ctx.fill(); } break;
      default: ctx.beginPath(); ctx.arc(p.x, p.y, p.size * this.dpr, 0, Math.PI * 2); ctx.closePath(); ctx.fill();
    }
    ctx.restore();
  }
  _linkParticles(ctx) {
    const opts = this.options.particles.line_linked; if (!opts.enable) return;
    const distMax = opts.distance * this.dpr; const cellSize = Math.max(1, distMax);
    const cols = Math.max(1, Math.ceil(this.canvas.width / cellSize)); const rows = Math.max(1, Math.ceil(this.canvas.height / cellSize));
    const grid = new Array(cols * rows); for (let i = 0; i < grid.length; i++) grid[i] = [];
    const parts = this.particles; for (let i = 0; i < parts.length; i++) { const p = parts[i]; const cx = Math.min(cols - 1, Math.max(0, Math.floor(p.x / cellSize))); const cy = Math.min(rows - 1, Math.max(0, Math.floor(p.y / cellSize))); grid[cy * cols + cx].push(i); }
    ctx.save(); ctx.lineWidth = opts.width * this.dpr; ctx.strokeStyle = this._rgba(opts.color, opts.opacity);
    for (let i = 0; i < parts.length; i++) { const a = parts[i]; const cx = Math.min(cols - 1, Math.max(0, Math.floor(a.x / cellSize))); const cy = Math.min(rows - 1, Math.max(0, Math.floor(a.y / cellSize)));
      for (let gx = cx - 1; gx <= cx + 1; gx++) { if (gx < 0 || gx >= cols) continue; for (let gy = cy - 1; gy <= cy + 1; gy++) { if (gy < 0 || gy >= rows) continue; const arr = grid[gy * cols + gx]; for (let k = 0; k < arr.length; k++) { const j = arr[k]; if (j <= i) continue; const b = parts[j]; const dx = a.x - b.x, dy = a.y - b.y; const d = Math.hypot(dx, dy); if (d <= distMax) { const alpha = Math.min(1, opts.opacity * (1 - d / distMax)); ctx.globalAlpha = alpha; ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke(); } } } }
    }
    ctx.restore();
  }
  _drawGrabLines(ctx) {
    const m = this.options.interactivity.modes.grab; const distMax = m.distance * this.dpr;
    ctx.save(); ctx.lineWidth = this.options.particles.line_linked.width * this.dpr; ctx.strokeStyle = this._rgba(this.options.particles.line_linked.color, m.line_linked.opacity);
    for (const p of this.particles) { const d = Math.hypot(p.x - this.pointer.x, p.y - this.pointer.y); if (d <= distMax) { const o = Math.min(1, m.line_linked.opacity * (1 - d / distMax)); ctx.globalAlpha = o; ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(this.pointer.x, this.pointer.y); ctx.stroke(); } }
    ctx.restore();
  }
  _applyClickMode(mode) {
    if (!mode) return; const modes = this.options.interactivity.modes; const cap = Math.max(1, this.options.particles.number.max ?? 300);
    switch (mode) {
      case 'push': { const want = modes.push.particles_nb; const free = Math.max(0, cap - this.particles.length); const add = Math.min(want, free); for (let i = 0; i < add; i++) this.particles.push(this._spawnParticle()); break; }
      case 'remove': { const n = Math.min(modes.remove.particles_nb, this.particles.length); this.particles.splice(0, n); break; }
      case 'repulse': for (const p of this.particles) { const dx = p.x - this.pointer.x; const dy = p.y - this.pointer.y; const d = Math.hypot(dx, dy) || 1; const force = Math.min(600, 6000 / d); p.vx += (dx / d) * force; p.vy += (dy / d) * force; } break;
      case 'bubble': break; // handled in update via click timing
    }
  }
  _resize() {
  ...
  _merge(a, b) {
    ...
    if (typeof a === 'object' && a !== null && typeof b === 'object' && b !== null) {
      const out = { ...a }; for (const k of Object.keys(b)) out[k] = this._merge(a[k], b[k]); return out;
    }
    return b ?? a;
  }
  ...
  _pickColor(value) { if (Array.isArray(value)) return value[Math.floor(Math.random() * value.length)]; return /** @type {string} */ (value); }

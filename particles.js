/**
 * @fileoverview Canvas particle engine (IIFE build).
 *
 * This file defines a small particle engine rendered with the HTML5 Canvas 2D API.
 ...
 *
 * ## Density (deprecated)
 * Historically the engine supported `particles.number.density` to scale the particle count
 * by canvas area. This behavior is **disabled** in this build to prevent click-to-add
 * interactions from being effectively blocked by density-based pre-filling.
 * The engine now always uses `particles.number.value` (clamped by `particles.number.max`).
 *
 * @version 2026-01-05
 */
 ...
 * @property {number} value - Initial particle count.
 * @property {{enable?: boolean, value_area?: number}} [density] - Deprecated/ignored.
 * @property {number} [max] - Hard cap for total particle count.
 */

/**
 * @typedef {Object} Particle
 * @property {number} x - X position (in device pixels).
 * @property {number} y - Y position (in device pixels).
 * @property {number} vx - X velocity (device px/sec).
 * @property {number} vy - Y velocity (device px/sec).
 * @property {number} ax - X acceleration (device px/sec^2).
 * @property {number} ay - Y acceleration (device px/sec^2).
 * @property {number} size - Current size scalar (CSS px, multiplied by DPR during render).
 * @property {number} baseSize - Rest size used for easing back after bubble effects.
 * @property {number} opacity - Current opacity [0..1].
 * @property {number} baseOpacity - Rest opacity used for easing back after bubble effects.
 * @property {string} fillStyle - Fill color.
 * @property {string|null} strokeStyle - Stroke color or null.
 * @property {HTMLImageElement|null} image - Optional sprite image when using `shape.type='image'`.
 * @property {boolean} [_dead] - Internal flag for removal.
 */

/**
 * @typedef {Object} ParticleEngineOptions
 * @property {boolean} [retina_detect=true] - When true, scales rendering by devicePixelRatio.
 * @property {string} [background='transparent'] - Canvas background.
 * @property {{
 *   number: ParticleNumberOptions,
 *   color: {value: string|string[]},
 *   shape: {type: ParticleShapeType, stroke: {width:number, color:string}, polygon: {nb_sides:number}, image: {src:string, width:number, height:number}},
 ...
 *   physics: {drag:number, gravity:number},
 *   move: {enable:boolean, speed:number|number[], direction:MoveDirection, random:boolean, straight:boolean, out_mode:'out'|'bounce', bounce:boolean, attract:{enable:boolean, rotateX:number, rotateY:number}, behavior:MoveBehavior, rocket:{thrust:number}, slide:{stiffness:number}, swirl:{strength:number}}
 * }} particles
 * @property {{
 *   detect_on:'container'|'canvas'|'window'|HTMLElement,
 *   events:{onhover:{enable:boolean, mode:InteractivityMode}, onclick:{enable:boolean, mode:InteractivityMode|null}, resize:boolean},
 *   modes:{grab:{distance:number, line_linked:{opacity:number}}, bubble:{distance:number, size:number, duration:number, opacity:number, speed:number}, repulse:{distance:number, duration:number}, push:{particles_nb:number}, remove:{particles_nb:number}, explode:{power:number}, rocketBoost:{power:number}}
 * }} interactivity
 */
...
(function (global) {
  'use strict';
  /**
 * Holds a named particle scene configuration.
 * @class ParticleScene
 * @param {string} name - Scene name.
 * @param {Object|function(ParticleJS):Object} config - Plain options or a function that returns options using the current engine.
 * @method toOptions
 * @param {ParticleJS} engine - Engine instance (provided when using a functional scene).
 * @returns {Object} - Resolved options for the scene.
 */
class ParticleScene { constructor(name, config) { this.name = name; this.config = config; } toOptions(engine) { return (typeof this.config === 'function') ? this.config(engine) : this.config; } }
  /**
